...
const ExceptionDashboard = ({ exception, onClose }) => {
    ...
    useEffect(() => {
        if (!exception || !exception.id) return;

        const EXCEPTION_ID = exception.id;

        // Fetch Root Cause (Description)
        const fetchRootCause = fetch(`/api/anomaly/root_cause/${EXCEPTION_ID}`).then(r => r.json());
        // Fetch Entities & KPIs
        const fetchEntities = fetch(`/api/anomaly/entities/${EXCEPTION_ID}`).then(r => r.json());
        // Fetch Alternatives
        const fetchAlts = fetch(`/api/anomaly/alternatives/${EXCEPTION_ID}`).then(r => r.json());
        ...
        Promise.all([fetchRootCause, fetchEntities, fetchAlts])
            .then(([rcData, entData, altsData]) => {
                ...
                setAnomaly({
                    id: EXCEPTION_ID,
                    title: exception.title || "Supply Chain Alert",
                    severity: exception.severity || "High",
                    description: rcData.root_cause ? rcData.root_cause : "A critical supply chain exception has been detected.",
                    affected_products: affectedProds.length > 0 ? affectedProds : ["Multiple Impacted Areas"],
                    impacted_orders_count: entData.magnitude || "Multiple",
                    estimated_value_at_risk: "TBD"
                });

                const kpis = entData.kpi_impact ? entData.kpi_impact.split(',') : ["Overall Delivery", "Cost"];
                const dynamicImpact = kpis.map(kpi => ({
                    kpi: kpi.trim(),
                    current: "Baseline",
                    predicted: "Risked",
                    delta: "Negative",
                    status: exception.severity === 'Critical' ? 'critical' : 'warning'
                }));

                setImpact(dynamicImpact);
